...
The equation \eqref{eq: PDE Monge condition.} is nonlinear in $T$ making the analysis of the Monge's Problem really difficult. Moreover, the constraint makes this problem hard to handle since it is not close even under weak convergence. 
...
\begin{equation}
\Pi(\mu, \nu)=\braces{\gamma \in \PlanSp\parentheses{X\times Y}: \Tmeasure{\gamma}{\parentheses{\proj_x}}=\mu \ \mand \ \Tmeasure{\gamma}{\parentheses{\proj_y}}=\nu}
\end{equation}
...
\begin{align*}
	\gamma_{1,2}(\dx_1\dx_2)&=\gamma_{1,2}(\dx_1|x_2)\mu_2(\dx_2)\\
	\gamma_{2,3}(\dx_2\dx_3)&=\gamma_{2,3}(\dx_3|x_2)\mu_2(\dx_2),
\end{align*}
and reconstruct $\gamma_{1,2,3}$ as follows,
\begin{align*}
	\gamma_{1,2,3}(\dx_1\dx_2\dx_3)&=\gamma_{1,2}(dx_1|x_2)\mu(x_2)\gamma_{2,3}(\dx_2\dx_3)
\end{align*}
...
Let ${T_n}$ be an assignation map. We see that we can always find a cheaper map ${T_{n+1}}$ for any $n\in \Naturals$. This sequence of transportation maps converges weakly to the plan $\gamma_{T_n}\weakconvergence \gamma_{T}=\frac{\gamma_{T^+}}{2}+\frac{\gamma_{T^-}}{2}$. Where $T^{+}$ and $T^{-}$ are given by:
...
\begin{align}
	\Kantorovich&=\min_{\gamma}\parentheses{\int_{X\times Y} c\diff\gamma +\sup_{\phi, \psi}\left(\int_{X} \phi \dmu +\int_{Y} \psi\dnu - \int_{X\times Y}\parentheses{\phi(x)+\psi(y)}\diff\gamma\right)} \\
...
\inf_{\gamma}\int_{X\times Y}\parentheses{c(x,y)-\parentheses{\phi(x)+\psi(y)}} \diff \gamma =\begin{cases}
	...
	-\infty & \text{otherwise}.
 \end{cases}
 \label{eq: Constrain indicator 2}
\end{equation}
...
\begin{equation}
	\Kantorovich=\sup_{\phi, \psi}\parentheses{\int_{X}\phi\dmu+\int_{Y}\psi\dnu+\inf_{\gamma}\int_{X\times Y}\parentheses{c(x,y)-\parentheses{\phi(x)+\psi(y)}}\diff\gamma} \label{eq: sup-inf Kantorovich}
\end{equation}
...
An interesting property of $c-$concave transforms is that taking $c-$, $\bar c -$ and $c-$ concave transforms consecutively, is equivalent to applying just one a $c-$concave transform. Consider any $\phi:X\rightarrow \Real\cup\braces{-\infty}$, and take the $c\bar c c-$transform as follows,
	\begin{align*}
...
of something. Its convex version \ref{thm: Convex Envelope Theorem} needs the Hahn Banach theorem due to the fact that convex functions are not defined via $\sup$ of affine functions, but via the convexity inequality. 


\begin{lemma}[Improvement through c-transforms]
	Let $X$ and $Y$ be two Polish spaces. Let $c:X\times Y \rightarrow \Real$ be a real valued cost function bounded from below. Let $\phi:X\rightarrow \Real$ and $\psi:Y\rightarrow\Real$ be two bounded real valued functions such that $\forall (x,y)\in X\times Y$ we have $\phi(x)+\psi(y)\leq c(x,y)$. Then, 
	\begin{enumerate}
...
\begin{definition}[$c-$superdifferential]
...
We can prove duality between the two problems introducing a perturbation \eqref{eq: H perturbation}, and then using convex analysis to prove a relation between the dual and primal transport problems. \\
We can find an equivalent proof in \cite{Buttazzo2000MongeKantorovich} for cost functions satisfying the Lipschitz condition. As it is presented in \cite{Santambrogio2015OT} we do it for uniformly continuous cost functions, since we already have proved the existence of a minimizer.
\begin{definition}
	...
	\begin{equation}
	H_\gamma(p)=-\max\braces{\int_X\phi \dmu + \int_Y\psi \dnu: \quad \phi(x)+\psi(y)\leq c(x,y)-p(x,y)} \label{eq: H perturbation}
	\end{equation}	
\end{definition}
...
\end{lemma}
	...
	Thus, its corresponding optimal potentials $(\phi_{n_k}, \psi_{n_k})$ are also equicontinuous and bounded, with $\phi_{n_k}\rightarrow \phi$ and  $\psi_{n_k}\rightarrow \psi$ uniformly. The sequence satisfies $\phi_{n_k}(x)+\psi_{n_k}\leq c(x,y)-p_{n_k}(x,y)$, therefore the limit satisfies the same inequality, making it a feasible solution.
	Hence, 
	\begin{equation}
...
\end{align*}
Note that, if $\gamma\neq \MeasureSp_+(X\times Y)$, then there is $p\leq 0$ such that $\int p_0 \diff\gamma$, and one can take $\phi=0$, $\psi=0$, $p=c+np_0$, and, for $n\rightarrow \infty$, we get $H^*(\gamma)=+\infty$. In the other hand, $\MeasureSp_+(X\times Y)$, we should choose the largest possible $p$, that is 
$p(x,y)=c(x,y)-\phi(x)-\psi(y)$. This yields,
\begin{equation}
	H^*(\gamma)=\sup_{\psi, \phi} \int_{X\times Y} c(x,y)\diff\gamma.
\end{equation}
Therefore,
\begin{equation*}
	H^*(\gamma)=\sup_{\phi, \psi}\int_{X\times Y}c(x,y)\diff\gamma +\int_{\phi}\phi\dmu-\int_{\phi}\phi(x)\diff\gamma+\int_{Y}\phi\dnu-\int_{X\times Y}\psi(y)\diff\gamma,
\end{equation*}
this is exactly the same equation \eqref{eq: sup-inf Kantorovich}, then we can rewrite the constraints as follows,
\begin{equation}
	H^*(\gamma)=\begin{cases}
	\Kantorovich & \text{if }\gamma\in \Pi(\mu,\nu)\\
	+\infty & \text{otherwise}
	\end{cases}
\end{equation}
We have that, 
\begin{equation}
\max\eqref{eq: Transportation Dual problem}=-H(0)=-H^{**}(0),
\end{equation} 
since $H$ is convex and lower semicontinuous. Moreover,
 \begin{equation}
 H^{**}(0)=\sup\parentheses{\anglesbox{0,\gamma}-H^*(\gamma)=-\inf (H^*)}=-\min\eqref{eq: Kantorovich's Problem.}.
 \end{equation} 
 
 In this way we have duality between the two problems \eqref{eq: Kantorovich's Problem.} and \eqref{eq: Transportation Dual problem} if $c$ is uniformly continuous defined on the $X\times Y$ Cartesian product of compact subsets of two Polish spaces. Note that we did not rely on the concept of $c-$cyclically monotonicity for proving the equality between the two problems.
...
\begin{theorem}
Let $\mu$ and $\nu$ probability measures on a compact domain $\Omega\subset \Real^d$, there exists an optimal plan $\gamma$ for the cost $c(x,y)=h(\abs{x-y})$, with $h$ a strictly convex. The plan is unique and it has the form $\Tmeasure{(\id, T)}{\mu}$, provided $\mu$ is absolutely continuous and the boundary $\partial \Omega$ is negligible. Moreover, there exists a function $\phi$, such that $T$ and $\phi$ are related by,
\begin{equation}
	T(x)=x-\parentheses{\nabla h}^{-1}\parentheses{\nabla\phi(x)}
\end{equation}
\end{theorem}


\begin{theorem}[Distance between two Gaussians]
	The optimal transport plan for a cost function $c(x,y)=\abs{x-y}^2$ between two Gaussians in $\Real^d$ is a deterministic coupling, given by an affine map.
	That is, $\mu$ is a probability measure for the space $X=\Real^n$, defined by,
	\begin{equation*}
	\mu(A)=\frac{1}{\sqrt{2\pi\det(\pmb{\Sigma}_\mu)}}\int_{A} \mathsf{e}^{(\mathbf{x}-\mathbf{m}_\mu)^\top\pmb{\Sigma}_{\mu}^{-1}(\mathbf{x}-\mathbf{m}_\mu)}\diff\mathbf{x}, \quad \forall A\in X,
	\end{equation*}
	where $\pmb{\Sigma}_\mu$ is its covariance matrix and $\mathbf{m}_\mu$ is its first moment. Let $\nu$ be a probability measure for the space $Y=\Real^m$ defined by,
	\begin{equation*}
	\nu(B)=\frac{1}{\sqrt{2\pi\det(\pmb{\Sigma}_\nu)}}\int_{B} \mathsf{e}^{(\mathbf{y}-\mathbf{m}_\nu)^\top\pmb{\Sigma}_{\nu}^{-1}(\mathbf{y}-\mathbf{m}_\nu)}\diff\mathbf{y}, \quad \forall B\in Y	
	\end{equation*} 
	where $\pmb{\Sigma}_\mu$ is its covariance matrix and $\mathbf{m}_\mu$ is its first moment.
	
	Then the plan $\gamma$ that solves the Kantorovich's problem is given by a translation map, that is $\gamma=\Tmeasure{(\id, T)}{\mu}$, where $T$ is defined by,
	\begin{equation}
		T(x)=\mathbf{m}_{\nu}+\mathbf{A}\parentheses{\mathbf{x}-\mathbf{m}_{\mu}},\quad \mathbf{x}\in\Real^n,
	\end{equation}
	and $\mathbf{A}$ is given by,
	\begin{equation}
		\mathbf{A}=\pmb{\Sigma}_\mu^{-\frac{1}{2}}\parentheses{\pmb{\Sigma}_\mu^{\frac{1}{2}}\pmb{\Sigma}_\nu\pmb{\Sigma}_\mu^{\frac{1}{2}}}^{\frac{1}{2}}\pmb{\Sigma}_\alpha^{-\frac{1}{2}}.
	\end{equation}
\end{theorem}

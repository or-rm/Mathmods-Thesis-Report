...
\end{proof}
\begin{theorem}[Convexity of the cost function]
...
\begin{equation}
\Delta=\max\braces{\int_X\phi\dmu+\int_Y\psi \dnu: \phi \in C_b(X),\ \psi \in C_b(Y),\ \phi(x)+\psi(y)\leq c(x,y),\ \forall (x,y)\in X\times Y} \tag{DP} \label{eq: Transportation Dual problem}
\end{equation}	
\end{problem}
...
Since the set of admissible maps is not compact we cannot assure the existence of a maximizer. To tackle we need to characterize functions by means of $c-$concavity.
...
\begin{definition}
	...
	\end{equation}
	In similar way, we can define the $\bar{c}-$transform of $\xi: Y\rightarrow \Realex$ by
	\begin{equation}
	\xi^{\bar{c}}(x)=\inf_{y\in Y}\parentheses{c(x, y)-\xi(y)}
	\end{equation}
	
	A function $\psi$ defined on $Y$ is said to be $c-$concave if it is not identically to $-\infty$ and there exists $\omega$ defined on $X$, such that $\psi=\omega^c$. Similarly, a function $\phi$ defined on $X$ is said to be $\bar c-$concave if it is not identically to $-\infty$ and there exists $\xi$ defined on $Y$ such that $\phi=\xi^{\bar{c}}$.
	\\
	Using the above definitions we call the $c-$superdifferential of a function $\omega$ the $c$-cyclically monotone set,
	\begin{equation}
...
The subdifferentials  of convex functions on $\Real^n$ are characterized in terms of a monotonicity property.
...
\begin{theorem}
	If $\Gamma$ is not empty c-cyclically monotone set in $X\times Y$ and $c:X\times Y \rightarrow \Real$, then there is a $c-$concave function $\phi:X\rightarrow \Real\cup\braces{-\infty}$ and not everywhere $-\infty$ such that,
	\begin{equation}
		\Gamma\subset\braces{(x,y)\in X\times Y:\quad \phi(x)+\phi^c(y)=c(x,y)}
	\end{equation}
\end{theorem}
\begin{theorem}
	Let $X$ and $Y$ be two Polish spaces. If $\gamma$ is an optimal transport plan for the cost $c:X\times Y \rightarrow \Real$ and $c$ is continuous then $\spt(\gamma)$ is a $c$-cyclical monotone set.
\end{theorem}
\begin{proof}
...
\begin{equation*}
0<2k\epsilon<\sum_{i}^{k}c(x_i, y_i)-c(x_i, y_{\sigma(i)}).
\end{equation*} 
Since $c$ is continuous, there exists $r$ such that for any $i=1,\dots,k$ and any $\ball{x_i}{r}\times \ball{y_i}{r}$ we have $c(x_i, y_i)-c(x, y)<\epsilon$. Similarly, for all $(x,y)$ belonging to $\ball{x_i}{r}\times\ball{y_{\sigma(i)}}{r}$, we have $c(x,y)-c(x_i, y_{\sigma(i)})<\epsilon$.
...
\end{align*}
Note that $\gamma_i$ is concentrated on $V_i=\ball{x_i}{r}\times\ball{y_i}{r}$, and $\tilde{\gamma_i}$ is concentrated on $\ball{x_i}{r}\times\ball{y_{\sigma{i}}}{r}$. And both are probability measures, then we check have,
\begin{align*}
	...
	&\geq\epsilon_0\sum_{i=1}^{k}\int\parentheses{c(x_i, y_i)-\epsilon}\diff\gamma_i-\epsilon_0\sum_{i=1}^{k}\int \parentheses{c(x_i, y_{\sigma(i)})+\epsilon}\diff\tilde\gamma_i\\
	&=\epsilon_0\parentheses{\sum_{i=1}^{k}\parentheses{c(x_i, y_i)-\epsilon}-\sum_{i=1}^{k} \parentheses{c(x_i, y_{\sigma(i)})+\epsilon}}\\
	&=\epsilon_0\parentheses{\sum_{i=1}^{k}c(x_i, y_i)-\sum_{i=1}^{k} c(x_i, y_{\sigma(i)})-2k\epsilon}>0
\end{align*}
...
\begin{theorem}
	Suppose that $X$ and $Y$ are Polish spaces and that $c:X\times Y\rightarrow \Real$ is uniformly continuous  and bounded. Then the problem \eqref{eq: Transportation Dual problem} admits a solution $\left(\phi, \psi\right)$. Moreover $\psi=\phi^c$ and the objective value of \eqref{eq: Transportation Dual problem} is equal to the objective value of the Kantorovich's problem \eqref{eq: Kantorovich's Problem.}. 
\end{theorem}
\begin{proof}
	First consider the minimization problem \ref{eq: Kantorovich's Problem.}. Since $c$ is continuous, it
	admits a solution $\gamma$, and $\spt(\gamma)$ is a $c-$cyclically monotone set. Any $c-$cyclically monotone set is contained in the $c-$superdifferential of a $c-$concave function $\phi$. The uniform continuity of $c$ and the $c-$concavity (and $\bar{c}-$concavity) implies continuity of $\phi^c$. From $\phi^c(y)=\inf_x\parentheses{c(x,y)-\phi(x)}$ we obtain an upper bound on $\phi^c$ (since $c$ is bounded), which gives a lower bound for $\phi$. Symmetrically, we can also obtain upper bounds on $\phi$ and lower on $\phi^c$, which prove that $\phi$ and $\phi^c$ are bounded and continuous.
	Consider now,
	\begin{equation*}
		\int_X \phi \dmu +\int_y \phi^c\dnu=\int_{X\times Y}\parentheses{\phi+\phi^c}\diff\gamma=\int_{X\times Y}c(x,y)\diff \gamma
	\end{equation*} 
	This equality holds because of $\gamma$ being optimal is concentrated on a $c-$cyclically monotone set and there we have $\phi(x)+\phi^c(y)=c(x,y)$. 
	By definition,
	\begin{equation}
		\Delta \geq \int_{X}\phi\dmu +\int_{Y}\phi^c\dnu = \int_{X\times Y}c\diff\gamma=\min\eqref{eq: Kantorovich's Problem.}
	\end{equation}  
	Since $\max\eqref{eq: Transportation Dual problem}\leq \min\eqref{eq: Kantorovich's Problem.}$, we have the desired equality for both problems.
\end{proof}

If $c$ is lower semicontinuous we cannot assure the existence of a minimizer.
\begin{theorem}	
\end{theorem}
\begin{theorem}
\end{theorem}
...
\begin{definition}
	Suppose that $X$ and $Y$ are compact metric spaces and $c:X\times Y\rightarrow \Real$ is continuous. For every $p\in C(X\times Y)$, let $H:C(X\times Y)\rightarrow \Realex$ be a perturbation of the problem,
	\begin{equation}

...
\begin{lemma}
	Let $X$ and $Y$ be two Polish spaces. And let $c:X\times Y \rightarrow \Real$ be a uniform continuous function with continuity modulus $\zeta$. Then $\phi^c(x)$ and $\psi^{\bar{c}}$ are uniformly continuous with continuity modulus $\zeta$, for any $\phi\in C(X)$ and $\psi\in C(Y)$.
\end{lemma}
\begin{theorem}
